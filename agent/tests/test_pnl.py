import pandas as pd
import os
from agent.green.pnl_tracker import save_portfolio_weights, calculate_pnl
...
def test_pnl_flow():
    ...
    def mock_read_local_market_data(ticker, current_date_str):
        if ticker == 'AAPL':
            return mock_data_aapl
        if ticker == 'GOOGL':
            return mock_data_googl
        return []

    with patch('agent.tools.data_manager.read_local_market_data', side_effect=mock_read_local_market_data):
